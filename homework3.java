    ...
    static final class SymbolTable {
        ...
        public static SymbolTable generateSymbolTable(AST tree) {
            ...
            if (tree.value.equals("scope")) {
                ...
                hashtable2.get(lastProg.toUpperCase()).get(0).size=1;
...
//TO-DO:: (maybe) take care of gloab addr and bla bla bla
                    ...
                    if(tree.right.value.equals("identifier"))
                        type=tree.right.left.value;
                    else
                        type=tree.right.value;


                    Variable var = new Variable(type, ADR, global_offset, "1   ", 1, FunctionNum); //making a new variable instance, tree.right holds the variable's type
                    if (tree.value.equals("byReference")) {
                        var.byRef = true;
                    }
                    if (!hashtable2.containsKey(tree.left.left.value)) {
    ...
    private static int size_of(AST ast) {
        if (ast.value.equals("int") || ast.value.equals("bool") || ast.value.equals("real")) {
            ...
            Variable var = new Variable();
            int counter = 0;
            //System.out.printf("aaaaaa: %s \n", ast.left.value.toUpperCase());

            if(SymbolTable.hashtable2.containsKey((ast.left.value)))
    ...
    private static void generatePCode(AST ast, SymbolTable symbolTable) {
        ...
        if (ast.left.right != null) {
            ...
            SEP = 0;
            MAX_SEP = 0;
            declearFunctions(ast.left.right,symbolTable);
    ...
    private static void declearFunctions(AST ast, SymbolTable symbolTable) {
    	System.out.printf("%s in declear \n",ast.value);
    	if (ast.left != null) {
            
            	declearFunctions(ast.left, symbolTable);
            }
    	if (ast.right.right.left != null) {
            if (ast.right.right.left.right != null) {
            	declearFunctions(ast.right.right.left.right, symbolTable);
            }
        }
    ...
    private static void coder(AST ast, SymbolTable symbols) {
        ...
        if (ast.value.equals("call")) {
          ...
          //  System.out.printf("aaaaaaaaaaaaaaaaaaaaa:  %s\n", ast.left.left.value);

            if (!(symbols.hashtable2.containsKey(ast.left.left.value.toUpperCase()))) {
                Variable firstvar = new Variable();
                String first = ast.left.left.value;
                ...
                first = firstvar.type.toUpperCase();
              //  System.out.printf("aaaaaaaaadsfgaaaaaaaaaaaa:  %d\n", firstvar.addr);
                firstvar = symbols.hashtable2.get(ast.left.left.value.toUpperCase()).get(0);
                List<Variable> list = new ArrayList<>();
                list.add(firstvar);
                String a = ast.left.left.left.value.toUpperCase();
                System.out.printf("aaaaaaaaadsfgaaaaaaaaaaaa:  %s\n", ast.left.left.left.value.toUpperCase());
                symbols.hashtable2.put(ast.left.left.left.value.toUpperCase(), list);
            }
    ...
    private static void code(AST ast, SymbolTable symbols) {
        ...
        if (ast.value.equals("call")) {
            if(!(symbols.hashtable2.containsKey(ast.left.left.value.toUpperCase())))
            {
                ...
                firstvar=symbols.hashtable2.get(first).get(0);
                ...
                symbols.hashtable2.put(ast.left.left.value.toUpperCase(),list);
            }
    ...
    private static void sendArgs(AST ast, SymbolTable symbols) {
        ...
        if (ast.right.value.equals("identifier")) {
            ...
            boolean found = false;
            int counter = 0;
            if(!symbols.hashtable2.containsKey(ast.right.left.value))
            {
            	System.out.printf("%s is not in hashtable2 \n", ast.right.left.value);
                while (!found) {
                    for (int i = 0; i < symbols.hashtable2.get(ast.right.left.value.toUpperCase()).size(); i++) {
                        if ((symbols.hashtable2.get(ast.right.left.value.toUpperCase()).get(i).funcSequence - counter) == symbols.hashtable2.get(lastProg.toUpperCase()).get(0).funcSequence) {
                            System.out.printf("%s identifier\n",ast.right.left.value.toUpperCase());
                        	firstvar = symbols.hashtable2.get(ast.right.left.value.toUpperCase()).get(i);
                            found = true;
                        }
                    }
                    counter++;
            ...
            System.out.printf("%d identifier\n",symbols.hashtable2.get(ast.right.left.value).size());
            while (!found) {
                for (int i = 0; i < symbols.hashtable2.get(ast.right.left.value).size(); i++) {
